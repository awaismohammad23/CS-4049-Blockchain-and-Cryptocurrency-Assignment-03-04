...
contract FASTCOIN is ERC20 {
    ...
    constructor (uint _numTokens , address _addraccesscontrol) {
    ...
    modifier onlyAdmin() {
        require(msg.sender == AC.universityAdmin(),"Only owner can approve someone for tokens");
        _;
    }
    function totalSupply() external view returns (uint256){
        return numTokens;
    ...
    function approve(address spender, uint256 value) external onlyAdmin returns (bool){
    ...
    modifier CafeOwner() {
        require(AC.isCafeAdmin(msg.sender) , "You can only send money to cafe owners.");
        _;
    }
    function transferFrom(address sender, address recipient, uint256 amount) external CafeOwner returns
    (bool){
