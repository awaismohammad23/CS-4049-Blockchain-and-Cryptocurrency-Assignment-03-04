...
contract OrderProcessingContract{
    ...
    constructor(address _addrmenu , address _addrdiscandpro , address _addraccesscontrol){
    ...
    function CalculateOrderAmount(string [] memory itemnames , uint [] memory quantities) external returns(uint /*, string [] memory ,uint [] memory*/){
        uint totalAmount;
        string [] memory names = mn.getItemNames();
        uint [] memory price = mn.getMenuPrice();
        uint [] memory quantity = mn.getMenuQuantity();
        ...
        for (uint i = 0 ; i < names.length ; i++){
            if (keccak256(abi.encodePacked(names[i])) == keccak256(abi.encodePacked(itemnames[j]))) {
                    ...
                    if (typs == 1){
                        mn.setQuantity(quantity[i] - quantities[j] - quantities[j]/qt , i , names[i]);
                        quantities[j]+=(quantities[j]/qt);
                        totalAmount += quantities[j] * price[i];
                    } else if (typs == 2) {
                        mn.setQuantity(quantity[i] - quantities[j] , i , names[i]);
                        totalAmount += quantities[j] * (prcper / 100 * price[i]);
                    }
                    j++;
            }
        }
        return totalAmount /*, itemnames , quantities)*/ ;
    }
}